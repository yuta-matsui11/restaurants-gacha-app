...
public class HistoryController {
     ...
     */
    @PostMapping("/save")
    public ResponseEntity<GachaHistory> saveHistory(
            @RequestParam Long user_id,
            @RequestParam String station_name,
            @RequestBody Dtos.GachaResponse gachaResult) {
        
        GachaHistory savedHistory = historyService.saveHistory(user_id, station_name, gachaResult);
    ...
    public ResponseEntity<List<GachaHistory>> getHistoryByUserId(@PathVariable Long user_id) {
        List<GachaHistory> historyList = historyService.getHistoryByUserId(user_id);
